"use strict";
/* ============================================================
   NOVOCAINE TRAINING — standalone build
   Ported from the Claude artifact for GitHub Pages / PWA install.
   Three swaps only, no behavior changes: icons (was lucide-react)
   are inline SVG, the TM chart (was recharts) is hand-rolled SVG,
   and storage (was the Claude artifact window.storage API) is
   localStorage. Everything else is untouched from the tested source.
   ============================================================ */
...
/* ============================================================
   ...
   Built from Justin's two StrongFirst articles. Core logic is
   computed from first principles (validated against the source
   tables), not hardcoded per-week lookups. See SETTINGS for
   every assumption that wasn't explicit in the source text.
   ============================================================ */
...
// Mesocycle x microcycle wave — validated against Image 2, flattens to L,M,L,M,M,L,H,H,M
...
// Weighted Pull-Up — always paired with S&S strength days, prescribed as a ladder rather than %TM
...
const DEFAULT_SETTINGS = {
    ...
    lissShort: { L: 40, M: 60, H: 80 }, // sessions attached to non-terminal LISS slots
    lissLong: { L: 60, M: 90, H: 120 }, // the "cap" LISS at the end of a 7-day pass
    // VDOT progression now uses whole-point steps from Threshold-supplement RPE — see resolveTmRows, reused for both.
    exerciseNames: {
...
const DEFAULT_PROGRESS = { passIndex: 0, sessionIndexInPass: 0, cycleCount: 0, rpeThisCycleBench: [], rpeThisCycleSquat: [], rpeThisCycleLM: [], rpeThisCycleThreshold: [], rpeThisCycleEasy: [] };
...
/* ---------- Daniels published VDOT training-pace table (VDOT 20-90) ----------
   [vdot, easySec/mi, easyMph@1%, thrSec/mi, thrMph@1%, repSec/mi, repMph@1%]
   Source: Daniels' Running Formula VDOT tables (30-85 published; 20-29 and 86-90
   are linear extrapolations of the curve). Treadmill mph at 1% incline per ACSM:
   treadmill speed = outdoor speed ÷ 1.045. Pace prescriptions are interpolated
   directly from this table rather than re-derived via the VO2 formula, which
   runs measurably fast at higher VDOT relative to Daniels' actual published paces. */
const VDOT_TABLE = [
...
/* ---------- Training Max percentage table (95-455 lb, 5 lb rows) ----------
   [TM, 90%, 82.5%, 80%, 75%, 70%, 60%, 50%] — all values pre-rounded to
   nearest 5 lb, taken directly from the uploaded reference chart. Strength
   loads are looked up from this table, not recomputed via rounding math, so
   the app always matches the chart exactly. Progression/regression steps
   move by table ROWS (each row = 5 lb), not by percentage. */
const TM_TABLE = [
    [95, 85, 80, 75, 70, 65, 55, 50], [100, 90, 85, 80, 75, 70, 60, 50], [105, 95, 85, 85, 80, 75, 65, 55],
    [110, 100, 90, 90, 85, 75, 65, 55], [115, 105, 95, 90, 85, 80, 70, 60], [120, 110, 100, 95, 90, 85, 70, 60],
    [125, 115, 105, 100, 95, 90, 75, 65], [130, 115, 105, 105, 100, 90, 80, 65], [135, 120, 110, 110, 100, 95, 80, 70],
    [140, 125, 115, 110, 105, 100, 85, 70], [145, 130, 120, 115, 110, 100, 85, 75], [150, 135, 125, 120, 115, 105, 90, 75],
    [155, 140, 130, 125, 115, 110, 95, 80], [160, 145, 130, 130, 120, 110, 95, 80], [165, 150, 135, 130, 125, 115, 100, 85],
    [170, 155, 140, 135, 130, 120, 100, 85], [175, 160, 145, 140, 130, 125, 105, 90], [180, 160, 150, 145, 135, 125, 110, 90],
    [185, 165, 155, 150, 140, 130, 110, 95], [190, 170, 155, 150, 145, 135, 115, 95], [195, 175, 160, 155, 145, 135, 115, 100],
    [200, 180, 165, 160, 150, 140, 120, 100], [205, 185, 170, 165, 155, 145, 125, 105], [210, 190, 175, 170, 160, 145, 125, 105],
    [215, 195, 175, 170, 160, 150, 130, 110], [220, 200, 180, 175, 165, 155, 130, 110], [225, 205, 185, 180, 170, 160, 135, 115],
    [230, 205, 190, 185, 175, 160, 140, 115], [235, 210, 195, 190, 175, 165, 140, 120], [240, 215, 200, 190, 180, 170, 145, 120],
    [245, 220, 200, 195, 185, 170, 145, 125], [250, 225, 205, 200, 190, 175, 150, 125], [255, 230, 210, 205, 190, 180, 155, 130],
    [260, 235, 215, 210, 195, 180, 155, 130], [265, 240, 220, 210, 200, 185, 160, 135], [270, 245, 225, 215, 205, 190, 160, 135],
    [275, 250, 225, 220, 205, 195, 165, 140], [280, 250, 230, 225, 210, 195, 170, 140], [285, 255, 235, 230, 215, 200, 170, 145],
    [290, 260, 240, 230, 220, 205, 175, 145], [295, 265, 245, 235, 220, 205, 175, 150], [300, 270, 250, 240, 225, 210, 180, 150],
    [305, 275, 250, 245, 230, 215, 185, 155], [310, 280, 255, 250, 235, 215, 185, 155], [315, 285, 260, 250, 235, 220, 190, 160],
    [320, 290, 265, 255, 240, 225, 190, 160], [325, 295, 270, 260, 245, 230, 195, 165], [330, 295, 270, 265, 250, 230, 200, 165],
    [335, 300, 275, 270, 250, 235, 200, 170], [340, 305, 280, 270, 255, 240, 205, 170], [345, 310, 285, 275, 260, 240, 205, 175],
    [350, 315, 290, 280, 265, 245, 210, 175], [355, 320, 295, 285, 265, 250, 215, 180], [360, 325, 295, 290, 270, 250, 215, 180],
    [365, 330, 300, 290, 275, 255, 220, 185], [370, 335, 305, 295, 280, 260, 220, 185], [375, 340, 310, 300, 280, 265, 225, 190],
    [380, 340, 315, 305, 285, 265, 230, 190], [385, 345, 320, 310, 290, 270, 230, 195], [390, 350, 320, 310, 295, 275, 235, 195],
    [395, 355, 325, 315, 295, 275, 235, 200], [400, 360, 330, 320, 300, 280, 240, 200], [405, 365, 335, 325, 305, 285, 245, 205],
    [410, 370, 340, 330, 310, 285, 245, 205], [415, 375, 340, 330, 310, 290, 250, 210], [420, 380, 345, 335, 315, 295, 250, 210],
    [425, 385, 350, 340, 320, 300, 255, 215], [430, 385, 355, 345, 325, 300, 260, 215], [435, 390, 360, 350, 325, 305, 260, 220],
    [440, 395, 365, 350, 330, 310, 265, 220], [445, 400, 365, 355, 335, 310, 265, 225], [450, 405, 370, 360, 340, 315, 270, 225],
    [455, 410, 375, 365, 340, 320, 275, 230],
];
const TM_PCT_COL = { 90: 1, 82.5: 2, 80: 3, 75: 4, 70: 5, 60: 6, 50: 7 };
function tmRowIndex(tm) {
    const clamped = Math.max(TM_TABLE[0][0], Math.min(TM_TABLE[TM_TABLE.length - 1][0], tm || 0));
    let best = 0, bestDiff = Infinity;
    for (let i = 0; i < TM_TABLE.length; i++) {
        const diff = Math.abs(TM_TABLE[i][0] - clamped);
        if (diff < bestDiff) {
            bestDiff = diff;
            best = i;
        }
...
function tmTableLookup(tm, pct) {
    const col = TM_PCT_COL[pct];
    if (!tm || !col)
        return null;
    return TM_TABLE[tmRowIndex(tm)][col];
}
function stepTmRows(tm, rows) {
    const idx = tmRowIndex(tm);
    const nextIdx = Math.max(0, Math.min(TM_TABLE.length - 1, idx + rows));
    return TM_TABLE[nextIdx][0];
}
/* ---------- Progression rule, shared by TM (rows) and VDOT (whole points) ----------
   Both use the identical RPE tiering — only the unit differs (5lb table row vs 1 VDOT point):
   RPE 8-9  -> standard advance, +1
   RPE 6-7  -> felt too easy,     +2
   RPE <=5  -> felt very easy,    +3
   RPE >=10 -> at/beyond max effort, hold (0) — flagged, not auto-decreased
   Mixed RPEs are resolved by their average. */
function resolveTmRows(avgH) {
    if (avgH >= 10)
        return { rows: 0, label: 'Hold — at or beyond max effort' };
    if (avgH >= 8)
        return { rows: 1, label: 'Standard advance (RPE 8–9)' };
    if (avgH >= 6)
        return { rows: 2, label: 'Felt easy (RPE 6–7)' };
    return { rows: 3, label: 'Felt very easy (RPE ≤5)' };
}
// Bench and Front Squat progress independently — each pulls only its own High-week RPE
// (the S&S day it's actually attached to), not a pooled average across both lifts.
function resolveLiftRows(avg) {
    if (avg == null)
        return { rows: 0, label: 'No session logged this cycle', noData: true };
    return resolveTmRows(avg);
}
/* ---------- Category 3 VDOT rule (no Threshold/Tempo session exists in cat 3) ----------
   Uses Extended Aerobic (Easy-pace, target RPE 3-4) High-week RPE instead:
   avg 3-4  -> on target,        +1 VDOT
   avg <3   -> too easy for the target pace, +2 VDOT
   avg >4   -> running harder than "easy" should feel — don't auto-advance,
               prompt a manual VDOT check instead (paces may be miscalibrated). */
function resolveEasyVdotRule(avgEasy) {
    if (avgEasy == null)
        return { points: 0, label: 'No Extended Aerobic RPE logged this cycle', promptManual: false, noData: true };
    if (avgEasy < 3)
        return { points: 2, label: 'Consistently under target (avg <3)', promptManual: false };
    if (avgEasy <= 4)
        return { points: 1, label: 'On target (RPE 3–4)', promptManual: false };
    return { points: 0, label: 'Consistently above target (avg >4)', promptManual: true };
}
// zone: 'easy' | 'threshold' | 'repetition' — interpolated from the table above
...
// Validated against the supplement table: cat1/4 letter=KB letter; cat2/3 letter=sequential occurrence within track.
...
function prescriptionFor(baseLetter, level, isTerminalLiss, settings, profile, slot) {
    const s = settings;
    if (baseLetter === 'LISS') {
...
function supplementPrescriptionFor(slot, settings, profile) {
    if (!slot || !slot.track)
        return null;
    const name = exerciseFor(slot, settings.exerciseNames);
    if (slot.track === 'strength') {
        if (slot.base !== 'A')
            return { name, mode: 'freeform' }; // Accessory Lifts / Loaded Carry — log your own numbers
        const tmKey = slot.aOccurrence === 1 ? 'bench' : 'frontSquat';
        return { name, tmKey, mode: 'strength', includesPullup: true };
    }
    const zone = slot.base === 'C' ? 'easy' : slot.base === 'B' ? 'repetition' : 'threshold'; // A (Tempo)→Threshold, B (Repeats)→Repetition, C (Extended Aerobic, after A+A)→Easy
...
/* ---------- S&S Set Tracker — swings, then get-ups (alternating side) ---------- */
function SsSetTracker({ totalSets }) {
    const [phase, setPhase] = useState('swings'); // 'swings' | 'getups' | 'done'
...
function Setup({ onComplete }) {
    ...
    const [vdotMode, setVdotMode] = useState('calc'); // 'calc' | 'manual'
    ...
    return (React.createElement("div", { className: "max-w-md mx-auto" },
        React.createElement("div", { className: "flex items-center gap-2 mb-6 font-mono text-[11px] uppercase tracking-widest" }, steps.map((label, i) => (React.createElement(React.Fragment, { key: label },
        ...
        React.createElement("div", { className: "flex justify-between mt-8" },
            ...
            step < 3 ? (React.createElement("button", { onClick: () => setStep(step + 1), className: "flex items-center gap-1 px-4 py-2 rounded-sm text-sm font-medium", style: { backgroundColor: '#c99a2e', color: '#14161a' } },
                ...
                }), className: "flex items-center gap-1 px-4 py-2 rounded-sm text-sm font-medium", style: { backgroundColor: '#c99a2e', color: '#14161a' } }, "Start Program")))));
}
/* ---------- Cycle Track (signature visual) ---------- */
...
/* ---------- Rollover Modal ---------- */
function RolloverModal({ avgBench, avgSquat, lmSuggest, avgThreshold, avgEasy, category, currentTm, currentVdot, onApply, onDismiss }) {
    const usesEasyPathway = category === 3;
    const thresholdResult = usesEasyPathway ? null : (avgThreshold != null ? resolveTmRows(avgThreshold) : null);
    const easyResult = usesEasyPathway ? resolveEasyVdotRule(avgEasy) : null;
    const vdotPoints = usesEasyPathway ? (easyResult ? easyResult.points : 0) : (thresholdResult ? thresholdResult.rows : 0);
    const benchResult = resolveLiftRows(avgBench);
    const squatResult = resolveLiftRows(avgSquat);
    const newBench = stepTmRows(currentTm.bench, benchResult.rows);
    const newSquat = stepTmRows(currentTm.frontSquat, squatResult.rows);
    const newVdot = currentVdot ? Math.max(0, currentVdot + vdotPoints) : currentVdot;
    const rowColorFor = (rows) => rows === 0 ? '#8b9078' : rows === 1 ? '#8fac6f' : rows === 2 ? '#c99a2e' : '#c76b52';
    const vdotColor = vdotPoints === 0 ? '#8b9078' : vdotPoints === 1 ? '#8fac6f' : vdotPoints === 2 ? '#c99a2e' : '#c76b52';
    return (React.createElement("div", { className: "fixed inset-0 flex items-center justify-center p-4 z-50", style: { backgroundColor: 'rgba(0,0,0,0.88)' } },
        React.createElement("div", { className: "rounded-sm p-6 max-w-sm w-full border", style: { backgroundColor: '#1d2019', borderColor: '#3d4130' } },
            ...
            React.createElement("div", { className: "rounded-sm p-3 mb-2 border", style: { borderColor: '#2c2f26', backgroundColor: '#14161a' } },
                React.createElement("div", { className: "text-[10px] font-mono uppercase tracking-widest mb-1", style: { color: '#8b9078' } }, "Bench Press"),
                benchResult.noData ? (React.createElement("div", { className: "text-xs font-mono", style: { color: '#8b9078' } }, benchResult.label)) : (React.createElement(React.Fragment, null,
                    React.createElement("div", { className: "font-mono text-sm mb-0.5", style: { color: rowColorFor(benchResult.rows) } },
                        benchResult.label,
                        " \u2014 ",
                        benchResult.rows === 0 ? 'no change' : `+${benchResult.rows} row${benchResult.rows > 1 ? 's' : ''} (${benchResult.rows * 5} lb)`),
                    React.createElement("div", { className: "text-xs font-mono", style: { color: '#8b9078' } },
                        "Avg RPE ",
                        avgBench.toFixed(1),
                        " \u00B7 ",
                        currentTm.bench,
                        " \u2192 ",
                        React.createElement("span", { style: { color: '#e7e4d9' } }, newBench))))),
            React.createElement("div", { className: "rounded-sm p-3 mb-3 border", style: { borderColor: '#2c2f26', backgroundColor: '#14161a' } },
                React.createElement("div", { className: "text-[10px] font-mono uppercase tracking-widest mb-1", style: { color: '#8b9078' } }, "Front Squat"),
                squatResult.noData ? (React.createElement("div", { className: "text-xs font-mono", style: { color: '#8b9078' } }, squatResult.label)) : (React.createElement(React.Fragment, null,
                    React.createElement("div", { className: "font-mono text-sm mb-0.5", style: { color: rowColorFor(squatResult.rows) } },
                        squatResult.label,
                        " \u2014 ",
                        squatResult.rows === 0 ? 'no change' : `+${squatResult.rows} row${squatResult.rows > 1 ? 's' : ''} (${squatResult.rows * 5} lb)`),
                    React.createElement("div", { className: "text-xs font-mono", style: { color: '#8b9078' } },
                        "Avg RPE ",
                        avgSquat.toFixed(1),
                        " \u00B7 ",
                        currentTm.frontSquat,
                        " \u2192 ",
                        React.createElement("span", { style: { color: '#e7e4d9' } }, newSquat))))),
            (benchResult.rows === 0 && !benchResult.noData) || (squatResult.rows === 0 && !squatResult.noData) ? (React.createElement("p", { className: "text-xs font-mono mb-3", style: { color: '#5a5f4c', marginTop: '-4px' } }, "RPE 10 means at or beyond max effort \u2014 holding rather than auto-decreasing. Adjust manually in Settings if a deload is warranted.")) : null,
            React.createElement("div", { className: "rounded-sm p-3 mb-3 border", style: { borderColor: usesEasyPathway && easyResult && easyResult.promptManual ? '#a8402f66' : '#2c2f26', backgroundColor: usesEasyPathway && easyResult && easyResult.promptManual ? '#a8402f14' : '#14161a' } }, category === 4 ? (React.createElement(React.Fragment, null,
                React.createElement("div", { className: "text-[10px] font-mono uppercase tracking-widest mb-1", style: { color: '#8b9078' } }, "VDOT"),
                React.createElement("div", { className: "text-xs font-mono", style: { color: '#8b9078' } }, "Category 4 never prescribes an endurance session \u2014 nothing to progress VDOT from. Adjust it manually in Settings if needed."))) : usesEasyPathway ? (React.createElement(React.Fragment, null,
                React.createElement("div", { className: "text-[10px] font-mono uppercase tracking-widest mb-1", style: { color: '#8b9078' } }, "VDOT \u2014 Extended Aerobic (Easy-pace) sessions only"),
                easyResult.noData ? (React.createElement("div", { className: "text-xs font-mono", style: { color: '#8b9078' } },
                    easyResult.label,
                    " \u2014 VDOT unchanged.")) : easyResult.promptManual ? (React.createElement(React.Fragment, null,
                    React.createElement("p", { className: "text-xs font-mono mb-1", style: { color: '#8b9078' } },
                        "Avg Easy-pace RPE: ",
                        React.createElement("span", { style: { color: '#e7e4d9' } }, avgEasy.toFixed(1))),
                    React.createElement("div", { className: "font-mono text-sm mb-1", style: { color: '#c76b52' } }, easyResult.label),
                    React.createElement("div", { className: "text-xs", style: { color: '#e7e4d9' } }, "\"Easy\" is running harder than it should \u2014 your VDOT-derived paces may be off. Not auto-adjusting; check and update VDOT manually in Settings."))) : (React.createElement(React.Fragment, null,
                    React.createElement("p", { className: "text-xs font-mono mb-1", style: { color: '#8b9078' } },
                        "Avg Easy-pace RPE: ",
                        React.createElement("span", { style: { color: '#e7e4d9' } }, avgEasy.toFixed(1))),
                    React.createElement("div", { className: "font-mono text-sm mb-1", style: { color: vdotColor } },
                        easyResult.label,
                        " \u2014 ",
                        vdotPoints === 0 ? 'no change' : `+${vdotPoints} VDOT`),
                    vdotPoints > 0 && (React.createElement("div", { className: "text-xs font-mono", style: { color: '#8b9078' } },
                        "VDOT ",
                        currentVdot.toFixed(1),
                        " \u2192 ",
                        React.createElement("span", { style: { color: '#e7e4d9' } }, newVdot.toFixed(1)))))))) : (React.createElement(React.Fragment, null,
                React.createElement("div", { className: "text-[10px] font-mono uppercase tracking-widest mb-1", style: { color: '#8b9078' } }, "VDOT \u2014 Threshold (Tempo) sessions only"),
                avgThreshold == null ? (React.createElement("div", { className: "text-xs font-mono", style: { color: '#8b9078' } }, "No Threshold-pace session logged this cycle \u2014 VDOT unchanged.")) : (React.createElement(React.Fragment, null,
                    React.createElement("p", { className: "text-xs font-mono mb-1", style: { color: '#8b9078' } },
                        "Avg Threshold RPE: ",
                        React.createElement("span", { style: { color: '#e7e4d9' } }, avgThreshold.toFixed(1))),
                    React.createElement("div", { className: "font-mono text-sm mb-1", style: { color: vdotColor } },
                        thresholdResult.label,
                        " \u2014 ",
                        vdotPoints === 0 ? 'no change' : `+${vdotPoints} VDOT`),
                    vdotPoints > 0 && (React.createElement("div", { className: "text-xs font-mono", style: { color: '#8b9078' } },
                        "VDOT ",
                        currentVdot.toFixed(1),
                        " \u2192 ",
                        React.createElement("span", { style: { color: '#e7e4d9' } }, newVdot.toFixed(1))))))))),
            lmSuggest && (React.createElement("div", { className: "rounded-sm p-3 mb-3 border", style: { borderColor: '#c99a2e55', backgroundColor: '#c99a2e14' } },
                React.createElement("div", { className: "text-xs", style: { color: '#e7e4d9' } },
                    "One or more Low/Medium-week sessions logged RPE \u22645 this cycle \u2014 consider bumping an ",
                    React.createElement("span", { style: { color: '#c99a2e' } }, "additional 1\u20132 rows"),
                    " on top of the above. Not applied automatically."))),
    ...
    return (React.createElement("div", { className: "fixed inset-0 flex items-start justify-center p-4 z-50 overflow-y-auto", style: { backgroundColor: 'rgba(0,0,0,0.88)' } },
        React.createElement("div", { className: "rounded-sm p-6 max-w-lg w-full my-8 border", style: { backgroundColor: '#1d2019', borderColor: '#3d4130' } },
            ...
            tab === 'progress' ? (React.createElement(TmProgressChart, { tmHistory: tmHistory })) : (React.createElement("div", { className: "space-y-5 text-sm" },
                ...
                React.createElement("div", null,
                    React.createElement("div", { className: "text-xs font-mono uppercase tracking-wide mb-1", style: labelStyle }, "Training Max & VDOT"),
                    !tmUnlocked ? (React.createElement("div", { className: "rounded-sm p-3 border", style: { borderColor: '#2c2f26', backgroundColor: '#14161a' } },
                        React.createElement("p", { className: "text-xs mb-2", style: { color: '#8b9078' } }, "These normally advance automatically from your logged RPE at the end of each cycle. Editing them directly overrides that progression."),
                        React.createElement("button", { onClick: () => setTmUnlocked(true), className: "w-full px-3 py-2 rounded-sm text-xs font-mono uppercase tracking-wide border", style: { borderColor: '#c99a2e55', color: '#c99a2e' } }, "Override Automatic Progression"))) : (React.createElement(React.Fragment, null,
                        React.createElement("div", { className: "grid grid-cols-2 gap-2 mb-3" }, [['bench', 'Bench'], ['frontSquat', 'Front Squat']].map(([k, label]) => (React.createElement("div", { key: k },
...
function NovocaineApp() {
    ...
    const basePrescription = prescriptionFor(currentBase, level, isTerminalLiss, settings, profile, currentSlot);
    const supp = supplementPrescriptionFor(currentSlot, settings, profile);
    ...
    function advance(rpe) {
        ...
        let rpeBucketBench = [...(progress.rpeThisCycleBench || [])];
        let rpeBucketSquat = [...(progress.rpeThisCycleSquat || [])];
        let rpeBucketLM = [...(progress.rpeThisCycleLM || [])];
        let rpeBucketThreshold = [...(progress.rpeThisCycleThreshold || [])];
        let rpeBucketEasy = [...(progress.rpeThisCycleEasy || [])];
        if (level === 'H' && rpe != null && supp && supp.mode === 'strength' && supp.tmKey === 'bench')
            rpeBucketBench.push(rpe);
        if (level === 'H' && rpe != null && supp && supp.mode === 'strength' && supp.tmKey === 'frontSquat')
            rpeBucketSquat.push(rpe);
        if ((level === 'L' || level === 'M') && rpe != null)
            rpeBucketLM.push(rpe);
        if (level === 'H' && rpe != null && supp && supp.mode === 'endurance' && supp.zone === 'threshold')
            rpeBucketThreshold.push(rpe);
        if (level === 'H' && rpe != null && supp && supp.mode === 'endurance' && supp.zone === 'easy')
            rpeBucketEasy.push(rpe);
        ...
        if (nextIdx >= baseList.length) {
            ...
            if (nextPass === 0) {
                ...
                triggerRollover = {
                    avgBench: avg(rpeBucketBench), avgSquat: avg(rpeBucketSquat),
                    lmSuggest: rpeBucketLM.some((r) => r <= 5),
                    avgThreshold: avg(rpeBucketThreshold), avgEasy: avg(rpeBucketEasy),
                };
                rpeBucketBench = [];
                rpeBucketSquat = [];
                rpeBucketLM = [];
        ...
        const newProgress = { passIndex: nextPass, sessionIndexInPass: nextIdx, cycleCount: nextCycle, rpeThisCycleBench: rpeBucketBench, rpeThisCycleSquat: rpeBucketSquat, rpeThisCycleLM: rpeBucketLM, rpeThisCycleThreshold: rpeBucketThreshold, rpeThisCycleEasy: rpeBucketEasy };
    ...
    function applyRollover(benchRows, squatRows, vdotPoints) {
        const nextProfile = {
            ...profile,
            tm: { bench: stepTmRows(profile.tm.bench, benchRows), frontSquat: stepTmRows(profile.tm.frontSquat, squatRows) },
            vdot: profile.vdot ? Math.max(0, profile.vdot + vdotPoints) : profile.vdot,
        };
    ...
    if (!loaded)
        return React.createElement("div", { className: "min-h-screen bg-[#14161a]" });
    return (React.createElement("div", { className: "min-h-screen bg-[#14161a] text-[#e7e4d9]", style: { fontFamily: "'Inter', system-ui, sans-serif" } },
        ...
        React.createElement("div", { className: "max-w-md mx-auto px-5 py-8" },
            React.createElement("div", { className: "flex items-center justify-between mb-8" },
                ...
                !needsSetup && (React.createElement("button", { onClick: () => setShowSettings(true) },
                    React.createElement(SettingsIcon, { size: 18, className: "text-[#8b9078]" })))),
            needsSetup ? (React.createElement(Setup, { onComplete: handleSetupComplete })) : (React.createElement("div", { className: "space-y-6" },
                ...
                React.createElement("div", { className: "border border-[#2c2f26] rounded-sm p-5", style: { background: `linear-gradient(135deg, ${BASE_INFO[currentBase].color}14, transparent)` } },
                    ...
                    React.createElement("p", { className: "text-xs text-[#8b9078] mb-4" }, BASE_INFO[currentBase].full),
                    React.createElement("div", { className: "font-mono text-3xl text-[#c99a2e] mb-1" }, basePrescription.headline),
                    React.createElement("p", { className: "text-sm text-[#8b9078] mb-4" }, basePrescription.detail),
                    ...
                    timerPhases && (React.createElement("div", { className: "border-t pt-3 mt-1", style: { borderColor: '#2c2f26' } },
                        React.createElement("button", { onClick: () => setShowTimer(true), className: "w-full flex items-center justify-center gap-1.5 rounded-sm py-2.5 text-sm font-medium border", style: { borderColor: '#c99a2e55', color: '#c99a2e' } },
                            ...
                            " Start Set Timer"))),
                    supp && (React.createElement("div", { className: "border-t border-[#2c2f26] pt-3 mt-1" },
                        React.createElement("div", { className: "text-[10px] font-mono uppercase tracking-widest text-[#8b9078] mb-1" },
                            "Supplement \u2014 ",
                            supp.mode === 'endurance' ? 'Endurance' : 'Strength',
                            " ",
                            currentSlot.subLetter),
                        React.createElement("div", { className: "text-sm text-[#e7e4d9]" }, supp.name),
                        supp.mode === 'strength' ? (React.createElement(React.Fragment, null,
                            React.createElement("div", { className: "font-mono text-sm text-[#c99a2e] mt-0.5" }, profile.tm[supp.tmKey]
                                ? `${settings.strengthScheme[level].sets}×${settings.strengthScheme[level].reps} @ ${tmTableLookup(profile.tm[supp.tmKey], settings.strengthScheme[level].pct)} (${settings.strengthScheme[level].pct}% TM)`
                                : 'Set training max in Settings'),
                            supp.includesPullup && (React.createElement("div", { className: "mt-2 pt-2 border-t border-[#2c2f2680]" },
                                React.createElement("div", { className: "text-sm text-[#e7e4d9]" }, "Weighted Pull-Up"),
                                React.createElement("div", { className: "font-mono text-sm text-[#c99a2e] mt-0.5" }, pullupLadderText(level)))))) : supp.mode === 'freeform' ? (React.createElement("textarea", { value: noteInput, onChange: (e) => setNoteInput(e.target.value), placeholder: "Log what you did \u2014 exercise, load, sets/reps...", rows: 2, className: "w-full mt-1.5 bg-[#14161a] border border-[#2c2f26] rounded-sm px-2.5 py-2 text-sm text-[#e7e4d9] focus:outline-none focus:border-[#c99a2e] resize-none" })) : (React.createElement("div", { className: "font-mono text-sm text-[#c99a2e] mt-0.5" }, (() => {
                    ...
                    !logging ? (React.createElement("div", { className: "flex gap-2 mt-5" },
                        React.createElement("button", { onClick: () => setLogging(true), className: "flex-1 flex items-center justify-center gap-1.5 bg-[#c99a2e] text-[#14161a] rounded-sm py-2.5 text-sm font-medium" },
                            React.createElement(Check, { size: 15 }),
                            " Log Complete"),
                        React.createElement("button", { onClick: skipSession, className: "px-3 border border-[#2c2f26] rounded-sm text-[#8b9078]" },
                            React.createElement(SkipForward, { size: 16 })))) : (React.createElement("div", { className: "mt-5 border-t border-[#2c2f26] pt-4" },
                        React.createElement("div", { className: "text-[10px] font-mono uppercase tracking-widest text-[#8b9078] mb-2" },
                            "RPE \u2014 ",
                            rpeInput),
                        React.createElement("input", { type: "range", min: 1, max: 10, value: rpeInput, onChange: (e) => setRpeInput(Number(e.target.value)), className: "w-full accent-[#c99a2e]" }),
                        React.createElement("div", { className: "flex gap-2 mt-3" },
                            React.createElement("button", { onClick: () => advance(rpeInput), className: "flex-1 bg-[#c99a2e] text-[#14161a] rounded-sm py-2.5 text-sm font-medium" }, "Save"),
                            React.createElement("button", { onClick: () => setLogging(false), className: "px-4 border border-[#2c2f26] rounded-sm text-[#8b9078] text-sm" }, "Cancel"))))),
                history.length > 0 && (React.createElement("div", null,
                    React.createElement("div", { className: "text-[10px] font-mono uppercase tracking-widest text-[#8b9078] mb-2" }, "History"),
                    React.createElement("div", { className: "space-y-1.5 max-h-64 overflow-y-auto" }, history.slice(0, 20).map((h, i) => (React.createElement("div", { key: i, className: "border-b border-[#2c2f26] pb-1.5" },
                        React.createElement("div", { className: "flex items-center justify-between text-xs" },
                            React.createElement("div", { className: "flex items-center gap-2" },
                                ...
                                React.createElement("span", { className: "text-[#8b9078]" }, new Date(h.date).toLocaleDateString())),
                            React.createElement("span", { className: "font-mono text-[#5a5f4c]" }, h.rpe != null ? `RPE ${h.rpe}` : 'skipped')),
                        h.note && React.createElement("div", { className: "text-[11px] text-[#8b9078] mt-1 pl-0.5" }, h.note))))))),
                saveError && React.createElement("p", { className: "text-xs text-[#a8402f]" }, "Couldn't save \u2014 your progress may not persist.")))),
...
ReactDOM.createRoot(document.getElementById('root')).render(React.createElement(NovocaineApp));
